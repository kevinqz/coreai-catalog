...
def _generate_swift_snippet(model: dict, artifact: dict) -> str:
    ...
    Selects the correct Apple/Core AI API pattern based on the runner type:
      - CoreAIRunner (LLMs)        → CoreAIKit ChatSession API
...
// 1. Create a speech model from the catalog identifier
//    (Core AI downloads and caches the .aimodel bundle automatically)
let transcriber = try await SpeechModel(catalog: "{model_id}")
...
{runtime_notes}"""
    ...
    # LLM runners (CoreAIRunner) → CoreAIKit ChatSession API
...
//
// ⚠️  Conceptual — see https://github.com/apple/coreai-models for complete
//     working examples of the ChatSession CoreAIKit API.

import CoreAIKit

// 1. Start a chat session using the catalog model identifier
//    (Core AI downloads and caches the .aimodel bundle automatically)
let chat = try await ChatSession(catalog: "{model_id}")

// 2. Send a prompt and get a response
let reply = try await chat.respond(to: "Hello, how are you?")
print(reply.content)

// 3. Multi-turn conversation — the session maintains state
let followUp = try await chat.respond(to: "Tell me more about that.")
print(followUp.content)

{runtime_notes}"""

    # Non-LLM models (stock-runner, etc.) → CoreAI AIModel GraphModel API
    return f"""// {name} — Core AI integration snippet
// Generated by coreai-catalog
// Source: https://huggingface.co/{repo}
//
