# Chat

**33 models** in the catalog with this capability.

## Task synonyms

`chat`, `classification`, `code agent`, `code completion`, `code generation`, `creative writing`, `function calling`, `instruct`, `instruction following`, `json generation`, `llm`, `math`, `math reasoning`, `multimodal chat`, `on-device rag`, `private chat`, `qa`, `question answering`, `rag`, `reasoning`, `sentiment analysis`, `summarization`, `summarize`, `text classification`, `text generation`, `text summarization`, `tool use`, `translation`, `voice assistant`, `writing`, `zero-shot classification`

## Models

| Model | Score | Parameters | Devices | License | Commercial | Benchmark | Source |
|---|---|---|---|---|---|---|---|
| [Qwen3 0.6B](../../catalog.yaml#L1) | 90 | 0.6B | 📱💻 | Apache-2.0 | ✅ likely | 📊 | 🍎 |
| [Qwen3 4B](../../catalog.yaml#L1) | 90 | 4B | 📱💻 | Apache-2.0 | ✅ likely | 📊 | 🍎 |
| [Qwen3 Enhancer (Huihui Qwen3-4B Abliterated v2)](../../catalog.yaml#L1) | 85 | 4B | 📱💻 | Apache-2.0 | ✅ likely | — | 🔗 |
| [gpt-oss-20B](../../catalog.yaml#L1) | 85 | 20B / ~13GB | 💻 | Apache-2.0 | ✅ likely | 📊 | 🍎 |
| [MiniCPM5-1B](../../catalog.yaml#L1) | 83 | 1.08B | 📱💻 | Apache-2.0 | ✅ likely | 📊 | 🐼 |
| [Mistral 7B v0.3](../../catalog.yaml#L1) | 80 | 7B | 💻 | Apache-2.0 | ✅ likely | 📊 | 🍎 |
| [Qwen3 8B](../../catalog.yaml#L1) | 80 | 8B | 💻 | Apache-2.0 | ✅ likely | 📊 | 🍎 |
| [FastContext-1.0-4B](../../catalog.yaml#L1) | 78 | 4B | 📱💻 | MIT | ✅ likely | 📊 | 🐼 |
| [Granite 4.0-H 1B](../../catalog.yaml#L1) | 78 | 1B | 📱💻 | Apache-2.0 | ✅ likely | 📊 | 🐼 |
| [Qwen3 1.7B](../../catalog.yaml#L1) | 78 | 1.7B | 📱💻 | Apache-2.0 | ✅ likely | — | 🍎 |
| [Qwen3.5-0.8B](../../catalog.yaml#L1) | 78 | 0.8B | 📱💻 | Apache-2.0 | ✅ likely | 📊 | 🐼 |
| [Qwen3.5-2B](../../catalog.yaml#L1) | 78 | 2B | 📱💻 | Apache-2.0 | ✅ likely | 📊 | 🐼 |
| [Gemma 3 12B IT](../../catalog.yaml#L1) | 70 | 12B | 💻 | Gemma Terms | ⚠️ check_license | 📊 | 🍎 |
| [Gemma 3 4B IT](../../catalog.yaml#L1) | 70 | 4B | 💻 | Gemma Terms | ⚠️ check_license | 📊 | 🍎 |
| [BitCPM-8B](../../catalog.yaml#L1) | 68 | 8B / ternary {-1,0,+1} | 📱💻 | Apache-2.0 | ✅ likely | 📊 | 🐼 |
| [Granite 4.0-H 350M](../../catalog.yaml#L1) | 68 | 350M | 📱💻 | Apache-2.0 | ✅ likely | — | 🐼 |
| [LFM2.5-1.2B-Instruct](../../catalog.yaml#L1) | 68 | 1.2B | 📱💻 | LFM Open License v1.0 | ⚠️ check_license | 📊 | 🐼 |
| [Nanbeige4.1-3B](../../catalog.yaml#L1) | 68 | 3.93B | 📱💻 | Apache-2.0 | ⚠️ check_license | 📊 | 🐼 |
| [Gemma 4 E2B](../../catalog.yaml#L1) | 63 | E2B | 📱💻 | Gemma Terms | ⚠️ check_license | 📊 | 🐼 |
| [Gemma 4 E4B](../../catalog.yaml#L1) | 63 | E4B | 📱💻 | Gemma Terms | ⚠️ check_license | 📊 | 🐼 |
| [LLaDA-8B dLLM](../../catalog.yaml#L1) | 63 | 8B | 💻 | MIT | ✅ likely | — | 🐼 |
| [Qwen3.6-27B](../../catalog.yaml#L1) | 63 | 27B | 💻 | Apache-2.0 | ✅ likely | 📊 | 🐼 |
| [Qwen3.6-35B-A3B](../../catalog.yaml#L1) | 63 | 35B / ~3B active | 💻 | Apache-2.0 | ✅ likely | 📊 | 🐼 |
| [RWKV-7 Goose 1.5B](../../catalog.yaml#L1) | 63 | 1.5B | 💻 | Apache-2.0 | ✅ likely | — | 🐼 |
| [Ornith 1.0 9B](../../catalog.yaml#L1) | 55 | 9B | 💻 | MIT | ✅ likely | — | 🐼 |
| [GLM-4.7-Flash](../../catalog.yaml#L1) | 53 | 30B / ~3B active | 💻 | MIT | ⚠️ check_license | 📊 | 🐼 |
| [Gemma 4 12B](../../catalog.yaml#L1) | 53 | 12B | 💻 | Gemma Terms | ⚠️ check_license | 📊 | 🐼 |
| [Gemma 4 31B](../../catalog.yaml#L1) | 53 | 31B | 💻 | Gemma Terms | ⚠️ check_license | 📊 | 🐼 |
| [LFM2.5-8B-A1B](../../catalog.yaml#L1) | 53 | 8B / ~1B active | 📱💻 | LFM Open License v1.0 | ⚠️ check_license | — | 🐼 |
| [Qwen2.5-1.5B-Instruct (fabric, int8)](../../catalog.yaml#L1) | 53 | 1.5B |  | apache-2.0 | ✅ likely | — | fabric |
| [Qwen3 0.6B (fabric, int8)](../../catalog.yaml#L1) | 53 | 0.6B |  | apache-2.0 | ✅ likely | — | fabric |
| [Youtu-LLM-2B](../../catalog.yaml#L1) | 53 | 2B | 📱💻 | Other | ⚠️ check_license | — | 🐼 |
| [Gemma 4 12B IT Multimodal (warshanks)](../../catalog.yaml#L1) | 48 | 12B | 💻 | Apache-2.0 | ⚠️ check_license | — | 🔗 |

## Install

```bash
coreai-catalog install official-qwen3-0-6b
```

## Related

- [Compare all chat models](../compare/chat.md)
- [Search by this capability](../../catalog.yaml) — `coreai-catalog search --capability chat`
