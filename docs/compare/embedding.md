# Comparison: embedding

Side-by-side comparison of all 2 model(s) with the `embedding` capability.

| Model | Family | Parameters | Precision | Devices | License | Runtime | Benchmark | Source |
|---|---|---|---|---|---|---|---|---|
| EmbeddingGemma 300M | Gemma | 300M | fp16 | iPhone/Mac | Gemma Terms | CoreAIRunner (stock) | — | 🐼 Zoo |
| Qwen3-Embedding 0.6B | Qwen | 0.6B | fp16 | iPhone/Mac | Apache-2.0 | CoreAIRunner (stock) | 12.0 milliseconds (M4 Max) | 🐼 Zoo |

> Generated automatically by `scripts/generate_compare.py` from `catalog.yaml` + `benchmarks.yaml`.

